...
const SingleProductPage = () => {
    ...
    const {name,productId, colorId, size} = useParams()
   ...
   const numberBag = useSelector(state => state?.cartReducer?.numberBag)
    ...
    useEffect(() => {
        ...
        dispatch(actions.cartAction.getNumberBag(numberBag))
        dispatch(actions.cartAction.getSize(size))
    }, [productId, colorId, numberBag, size])
    ...
    // const likedProducts = products?.filter(item => item?.name?.includes( nameList[4]) && item.name.includes(nameList[3]) )
    ...
    const bagHandler = () => {
        ...
        dispatch(actions.cartAction.addToShoppingBag(product, productId, colorId, size))
        setIsOpen(true)
    }
